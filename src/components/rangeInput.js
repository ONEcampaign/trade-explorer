...
export function rangeInput(options = {}) {
  ...
  ${(controls.track = html`<div class="range-track">
    ${(controls.zone = html`<div class="range-track-zone">
      ${(controls.range = html`<div class="range-select" style="background:${
  ...
  const dom = html`<div style="display: flex; align-items: left; justify-content: start; gap: 1em;">
    ${
      label
        ? html`<label style="font-size: var(--size-m); font-weight: bold;">${label}</label>`
        : ""
    }
    ${sliderContent}
  </div>`;
